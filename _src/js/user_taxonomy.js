...
jQuery(document).ready(function ($) {
    ...
    $('.user-profile-taxonomy').on('keyup', '.newtag', function () {
        ...
        let current_elem = $(this);
        let tag_input_value = current_elem.val().split(',');
        tag_input_value = $.trim(tag_input_value[tag_input_value.length - 1]);

        if (tag_input_value.length >= 2) {
            delay(function () {

                let tag_id = current_elem.attr('id');
                let tag_name = tag_id.split('new-tag-user_tag_');
    ...
    $('body').on('click', '.tag-suggestion li', function () {
        let current_elem = $(this);
        let taxonomy_name = '';
        let term = current_elem.html();
        let tag_checklist = current_elem.parent().siblings('.tagchecklist');
        let num = (tag_checklist.length);

        let taxonomy_id = current_elem.parent().siblings('.newtag').attr('id');
        if (taxonomy_id) {
    ...
    $('body').on('click', '.button.tagadd', function () {
        let current_elem = $(this);
        let sibling = current_elem.siblings('.newtag');
        let newtag_val = sibling.val();

        if (!newtag_val) return;

        newtag_val = newtag_val.split(',');

        let taxonomy_name = sibling.attr('id').split('new-tag-user_tag_');
        taxonomy_name = taxonomy_name[1];

        let tag_checklist = current_elem.siblings('.tagchecklist');

        let num = '';
        for (let $i = 0; $i < newtag_val.length; $i++) {
            num = (tag_checklist.length);
            let tag_html = '<div class="tag-hldr"><a href="#" class="term-link">' + newtag_val[$i] + '</a><span><a id="post_tag-' + taxonomy_name + '-check-num-' + num + '" class="ntdelbutton">&#10005;</a></span></div>';
    ...
    $('body').on('click', '.ntdelbutton', function () {
        let current_elem = $(this);
        let parent = current_elem.parents().eq(1);
        let term = parent.find('.term-link').html();
        let tags_list = current_elem.parents().eq(2).siblings('input[type="hidden"]');
    ...
    $('body').on('click', '.tagcloud-link.user-taxonomy', function (e) {
        ...
        $.post(ajaxurl, {'action': 'get-tagcloud', 'tax': tax}, function (r, stat) {
            ...
            $('a', r).click(function () {
                ...
                let term_name = current_elem.html();
                let tag_checklist = current_elem.parents().eq(2).siblings('.tagchecklist');
                let tax_input = current_elem.parents().eq(2).siblings('.newtag');

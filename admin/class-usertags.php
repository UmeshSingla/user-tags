...
/**
 * Handle add/edit/delete of taxonomy
 */
...
//		add_action( 'restrict_manage_users', array( $this, 'ut_users_filter' ) );
		 ...
		 */
		function ut_registered_taxonomy( $taxonomy, $object, $args ) {
		...
		function register_page() {
			if ( current_user_can( 'edit_users' ) ) {
				$this->settings_page = add_users_page(
					esc_html__( 'User Taxonomy', 'user_taxonomy' ), esc_html__( 'User Taxonomy', 'user_taxonomy' ), 'read', 'user-taxonomies', array(
						$this,
						'ut_user_taxonomies',
					)
				);
		...
		function ut_user_taxonomies() {
			...
			if ( ! empty( $_GET['taxonomy'] ) ) {
				$slug = sanitize_title( $_GET['taxonomy'] );

				$page_title = sprintf( esc_html__( 'Edit Taxonomy: %s', 'user_taxonomy' ), $slug );
				...
				if ( ! empty( $ut_taxonomies ) ) {
					foreach ( $ut_taxonomies as $ut_taxonomy ) {
						if ( $ut_taxonomy['slug'] == $slug ) {
							$desc = ! empty( $ut_taxonomy['description'] ) ? trim( $ut_taxonomy['description'] ) : '';
						}
					}
				}
			} ?>
							...
							<div class="form-wrap">
								...
								<form name="editusertaxonomy" id="editusertaxonomy" method="post" action="" class="validate">
									...
									<div class="form-field term-description-wrap">
										...
										<textarea name="description" id="description" rows="5" cols="40"><?php echo esc_html( $desc ); ?></textarea>
		...
		public function ut_save_profile( $user_id ) {
			...
			if ( empty( $_POST['user-tags'] ) || ! current_user_can( 'edit_user', $user_id ) ) {
				return;
			}
		...
		function ut_delete_taxonomy_callback() {
			...
			$ut_taxonomies = get_site_option( 'ut_taxonomies' );
			foreach ( $ut_taxonomies as $key => $tax_array ) {
				if ( $_POST['delete_taxonomy'] === ut_stripallslashes( $tax_array['slug'] ) ) {

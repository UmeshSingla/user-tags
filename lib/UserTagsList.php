...
class UserTagsList extends WP_List_Table {
    ...
    function get_views(){
        ...
        $views['all'] = "<a href='{$all_url }' {$class} >".__('All',WP_UT_TRANSLATION_DOMAIN)." (<span class='album-count-all'>".$this->rtmedia_get_album_count('all')."</span>)</a>";
        ...
        foreach ( $contexts as $context ){
            ...
            $views[$context] = "<a href='".strtolower( $$string )."' {$class} >".__($context,WP_UT_TRANSLATION_DOMAIN) ." (<span class='album-count-".strtolower( $context )."'>". $this->rtmedia_get_album_count( $context ) ."</span>) </a>";
        }
        return $views;
     }
